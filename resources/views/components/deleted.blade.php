                    ...
                    @if(!$componentData->isEmpty())
                        <div class="grid grid-cols-1 gap-8">
                            <div class="col-span-1 p-8 bg-[#14151F] rounded-md">
                                <div>
                                    <p class="text-white text-2xl font-bold mb-4">All Deleted Components</p>
                                </div>
                                <div class="overflow-x-auto">
                                    <div class="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
                                        <table class="min-w-full leading-normal">
                                            ...
                                            <tbody>
                                            @foreach($componentData as $componentItem)
                                                @if($componentItem->is_deleted === "1")
                                                    <tr>
                                                        <td class="px-5 py-5 bg-[#282a36] text-sm">
                                                            <div class="flex">
                                                                <div class="">
                                                                    <p class="text-white whitespace-no-wrap">{{ $componentItem->title }}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td class="px-5 py-5 bg-[#282a36] text-sm">
                                                            <p class="text-white whitespace-no-wrap">{{ $componentItem->updated_at }}</p>
                                                        </td>
                                                        <td class="px-5 py-5 bg-[#282a36] text-sm text-right flex justify-end items-center">
                                                            <form class="" action="{{ route('components.restore', $componentItem->id) }}" method="POST">
                                                                @method('POST')
                                                                @csrf
                                                            ...
                                                            <form class="ml-4" action="{{ route('components.destroy', $componentItem->id) }}" method="POST">
                                                                @method('DELETE')
                                                                @csrf
